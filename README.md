...
#### Работа со стикерами
Бот поддерживает приём и отправку стикеров. Если вы хотите отправить стикер, используйте ключ `{sticker=XXX}`, где XXX является id стикера. Можно указать несколько ключей со стикерами, будет выбран случайный.
> Бот ничего не отправит, если его у вас нет нужного стикера

Для того, чтобы бот ответил на стикер, в команде необходимо указать `sticker=YYY:XXX`, где YYY - номер набора, а XXX - id стикера.
